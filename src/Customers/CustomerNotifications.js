...
export default function CustomerNotifications() {
    ...
    const GetUserNotifications = () => {
        axios
            .get(config.APIACTIVATEURL + config.GETNOTIFICATIONBYDEVICE + "?EndDeviceId=" + localStorage.getItem("endDeviceId"), { ...headerconfig })
            .then((response) => {
                                ...
                                {notifications.length > 0 &&
                                    notifications.map((notification, index) => (
                                        <tr key={notification.notificationId}>
                                            ...
                                            <td>{moment(notification.createdDate).format("DD MMM YYYY")}</td>
                                        </tr>
                                    ))}
                            </tbody>
                        </table>
